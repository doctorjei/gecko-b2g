...
export var DownloadIntegration = {
  ...
  async _getDefaultDownloadDirectory() {
    ...
    if (preferredStorageName) {
      let volume = lazy.volumeService.getVolumeByName(preferredStorageName);
      if (volume && volume.state === Ci.nsIVolume.STATE_MOUNTED) {
        directoryPath = PathUtils.join(volume.mountPoint, "downloads");
    ...
    throw new Components.Exception(
      "No suitable storage for downloads.",
  ...
  async getSystemDownloadsDirectory() {
    ...
    if (AppConstants.platform == "android") {
      ...
      this._downloadsDirectory = this._getDefaultDownloadDirectory();
    } else {
      try {
